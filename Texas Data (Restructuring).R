...
dat_messy <- read.csv(file = "Texas General 2020.csv")
...
write.csv(clean_dat, file = "Texas General 2020 (Cleaned).csv")
...
dat_messy <- read.csv(file = "Texas General 2018.csv")
dat_messy <- dat_messy[,-1]
...
for(row in 1:nrow(dat)){
  ...
  county <- (row %/% 93) + 1
  clean_dat$County[candidate] <- dat[(county-1)*93 + 3, 1]
  
  if(dat[row,2] == "D" | dat[row,2] == "R"){candidate <- candidate + 1}
}


clean_dat <- clean_dat[1:1451,]

write.csv(clean_dat, file = "Texas General 2018 (Cleaned).csv")
...
dat_messy <- read.csv(file = "Texas Primaries 2016.csv")
dat_messy <- dat_messy[,-1]
...
for(row in 1:nrow(dat)){
  ...
  county <- (row %/% 120) + 1
  clean_dat$County[candidate] <- dat[(county-1)*120 + 3, 1]
  
  if(dat[row,2] == "D" | dat[row,2] == "R"){candidate <- candidate + 1}
}


clean_dat <- clean_dat[1:1977,]

write.csv(clean_dat, file = "Texas Primaries 2016 (Cleaned).csv")
